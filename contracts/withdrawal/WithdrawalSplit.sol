...
abstract contract WithdrawalSplit is Ownable, ReentrancyGuard {
    struct Beneficiary {
        address wallet;
    ...
    error ZeroBeneficiaryBasisPoints();

    receive() external payable {}

    modifier checkWithdrawalBasisPoints(
        address[] memory _wallets,
        uint256[] memory _basisPoints
    ) {
        if (_wallets.length != _basisPoints.length)
            revert ArrayLengthMismatch();
        if (_wallets.length == 0) revert ZeroArrayLength();
        for (uint256 i; i < _wallets.length; i++) {
            if (_wallets[i] == address(0)) revert ZeroBeneficiaryAddress();
            if (_basisPoints[i] == 0) revert ZeroBeneficiaryBasisPoints();
        }
        _;
    }
    ...
    function setBeneficiaries(
        address[] memory _wallets,
        uint256[] memory _basisPoints
    ) public onlyOwner checkWithdrawalBasisPoints(_wallets, _basisPoints) {
